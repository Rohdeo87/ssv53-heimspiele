...
def render_markdown(plan: dict[str, Any]) -> str:
    ...
    for day in plan["days"]:
        ...
        lines.append(f"### {WEEKDAY_DE[parsed_day.weekday()]}, {parsed_day.strftime('%d.%m.%Y')} – {hours} h {minutes:02d} min")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Berechnet den maximalen SSV53-Mähplan als Dry Run.")
    ...
    parser.add_argument("--start-date", default="", help="YYYY-MM-DD; Standard: heute in Europe/Berlin")
    parser.add_argument("--days", type=int, default=0, help="Planungstage; 0 nutzt die Konfiguration")
    parser.add_argument("--no-hydrawise", action="store_true", help="Hydrawise auch bei vorhandenem Secret nicht abrufen")
...
def main() -> int:
    ...
    start_day = date.fromisoformat(args.start_date) if args.start_date else datetime.now(tz).date()
    ...
    if not args.no_hydrawise and hydrawise_config.get("enabled", True) and api_key:
        controller_env = hydrawise_config.get("controller_id_env", "HYDRAWISE_CONTROLLER_ID")
        ...
        warnings.append("HYDRAWISE_API_KEY fehlt; Beregnungszeiten sind in diesem Dry Run noch nicht enthalten.")

    match_blocks = read_match_blocks(args.matches, tz)
    if not Path(args.matches).exists():
        warnings.append(f"Spielkalender {args.matches} fehlt; Heimspiele sind nicht enthalten.")

    plans, merged = create_plan(config, match_blocks, hydrawise_status, start_day, days)
    end_day = start_day.fromordinal(start_day.toordinal() + days - 1)
    metadata = {
        ...
        "days": days,
        "minimum_window_minutes": int(planning.get("minimum_mowing_window_minutes", 30)),
