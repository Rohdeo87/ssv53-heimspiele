...
from poc_scraper import Match, evaluate_quality, recalculate_event_times, write_ics, write_outputs
...
class Factory:
    ...
    def candidate(self, matches, at=NOW, *, config=None):
        ...
        write_outputs(source, matches, quality, {"teams": [], "changes": {}})
        write(source / "failed_teams.json", [])
        summary = read(source / "summary.json")
        summary["generated_at"] = at.isoformat()
        write(source / "summary.json", summary)
        config_path = source / "config.json"
        write(config_path, config)
        target = self.path("candidate")
        subprocess.run(
            [sys.executable, str(ROOT / "create_feed.py"), "--input", str(source),
             "--output", str(target), "--config", str(config_path)],
            check=True,
        )
        return target
...
def test_publication_preserves_held_duration_and_marks_ics(factory, config):
    old = game(config)
    before = factory.candidate([old], NOW - timedelta(hours=4))
    # A previously approved publication can carry a historical timing rule that
    # differs from today's candidate rule. Keep that old occupied interval.
    ...
    included = read(before / "included_matches.json")
    included[0].update(
        match_end=old.match_end,
        event_end=old.event_end,
        match_duration_minutes=old.match_duration_minutes,
        duration_rule=old.duration_rule,
    )
    write(before / "included_matches.json", included)
    feed = read(before / "matches.json")
    feed["matches"][0].update(
        end=old.match_end,
        occupancyEnd=old.event_end,
        matchDurationMinutes=old.match_duration_minutes,
        durationRule=old.duration_rule,
    )
    write(before / "matches.json", feed)
    write_ics(before / "rasen.ics", [old], "Rasen")
    output, _ = factory.publish(before, factory.candidate([game(config)]))
    held = holds(output)[0]
    assert held["matchDurationMinutes"] == 105
    blocks = read_match_blocks(output / "rasen.ics", TZ)
    ...
    assert held_block.title.startswith("Sperre bis Klärung:")
...
def test_versioned_consumer_contract_accepts_legacy_and_retained_rows(factory, config):
    before = factory.candidate([game(config)], NOW - timedelta(hours=4))
    assert len(decode_included_matches(read(before / "included_matches.json"))) == 1
    output, _ = factory.publish(before, factory.candidate([game(config, "NEW")]))
    included = read(output / "included_matches.json")
    assert len(decode_included_matches(included)) == 2
    included["requiredConsumer"] = "unknown-future-version"
    with pytest.raises(PublicationError, match="anderen geprüften Consumer"):
        decode_included_matches(included)
