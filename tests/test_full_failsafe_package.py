...
class FullFailsafePackageTests(unittest.TestCase):
    def test_package_contains_only_scoped_write_modules_and_locked_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("mower/onsite_dock_proof.py", names)
            self.assertFalse(manifest["onsite_dock_confirmation_default_enabled"])
            self.assertEqual(manifest["onsite_dock_confirmation_contract_version"], 4)
            self.assertTrue(manifest["onsite_dock_live_validation_required"])
