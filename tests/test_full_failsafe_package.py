...
class FullFailsafePackageTests(unittest.TestCase):
    def test_package_contains_only_scoped_write_modules_and_locked_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(manifest["trainer_collision_notifications_implemented"])
            self.assertTrue(manifest["trainer_contact_registration_write_only"])
            self.assertTrue(manifest["trainer_contact_mail_verification_required"])
